(function (root, factory) {
  ...
  function parseListingBlock(rawText) {
    ...
    const 감정가Label = extractLabelValue(text, "감정가");
    const 최저가Label = extractLabelValue(text, "최저가");
    const 감정가 = toNumberString(감정가Label);
    const 최저가 = toNumberString(최저가Label);
    ...
    const 소재지Label = extractLabelValue(text, "소재지");
    const 도로명주소 = parseRoadAddress(text);
    const 물건요약Label = extractLabelValue(text, "물건요약");

    const buildingSqm = toNumberString(extractLabelValue(text, "건물㎡") || extractLabelValue(text, "건물m2"));
    const buildingPyeong = toNumberString(extractLabelValue(text, "건물평"));
    const landSqm = toNumberString(extractLabelValue(text, "토지㎡") || extractLabelValue(text, "토지m2"));
    const landPyeong = toNumberString(extractLabelValue(text, "토지평"));
